...
var userExistSchema = new mongoose.Schema({
    ...
    emailAvailable          : { type: Number, default: 1 },
...
userExistSchema.statics.existCheck = function(email, id, callback) {
    var where = {};
    if (id) {
        where = {
    ...
    UserExist.findOne(where, function(err, userdata) {
        if (err) {
            callback(err)
        } else {
            if (userdata) {
...
userExistSchema.statics.isRegisteredExistCheck = function(email, id, callback) {
    var where = {};
    if (id) {
        where = {
            $or: [{ email: new RegExp('^' + email + '$', "i") }],
...
var UserExist = mongoose.model('userExist', userExistSchema);
