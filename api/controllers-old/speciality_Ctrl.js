...
function addSpeciality(req, res) {
    req.body.specialityName = (req.body.specialityName)?req.body.specialityName:'';
    req.body.searchKey      = (req.body.specialityName)?req.body.specialityName.toLowerCase().replace(/\s+/g, ''):'';
    specialityModel.findOne({ specialityName: { $regex: new RegExp(req.body.specialityName, "i") }, isDeleted: false })
    .lean()
    .exec(function(err, specialityData) {
        if (err) {
            ...
            res.json({ code: 202, message: 'Entered speciality name already exist! Try with different name.' });
        } else {
...
function getSpecialityList(req, res) {
    ...
    SpecialityModel.find(condition)
        ...
        .lean().exec(function(err, speciality) {
            if (err) {
                res.json({
                    code    : 401,
                    message : 'Internal error'
...
function getSpecialityById(req, res) {
    ...
    specialityModel.findOne({ _id: mongoose.Types.ObjectId(id) }, { specialityCode: 1, specialityDescp: 1, specialityName: 1 })
        .lean()
        .exec(function(err, specialityInfo) {
            if (err) {
                res.json({ code: 201, message: 'Request could not be processed. Please try again.', data: err });
            } else {
                res.json({ code: 200, message: 'Speciality info fetched successfully.', data: specialityInfo });
            }
        });
}
...
function updateSpecialityByAdmin(req, res) {
    var updateSpecialityRecord = {
                                    ...
                                    searchKey       : (req.body.specialityName)? req.body.specialityName.toLowerCase().replace(/\s+/g, ''):''
                                }
    var thename =  '^req.body.specialityName$';
    specialityModel.findOne({ specialityName:  req.body.specialityName , isDeleted: false, _id: { $ne: req.body._id } }).exec(function(err, specialityData) {
        if (err) {
            res.json({
                code    : 201,
                message : 'Internal error'
            ...
            specialityModel.update({ _id: mongoose.Types.ObjectId(req.body._id) }, { $set: updateSpecialityRecord }, function(err) {
                if (err) {
                    res.json({ code: 201, message: 'Request could not be processed. Please try again.' });
                } else {
                    res.json({ code: 200, message: 'Speciality updated successfully.' });
                }
            })
        }
    })
...
function deleteSpeciality(req, res) {
    specialityModel.update({ _id: mongoose.Types.ObjectId(req.body.id) }, { $set: { isDeleted: true } }, function(err) {
        if (err) {
            res.json({ code: 201, message: 'Request could not be processed. Please try again.' });
        } else {
...
function updateSpecialityStatus(req, res) {
    ...
    specialityModel.update({ _id: mongoose.Types.ObjectId(req.body.id) }, { $set: updateSpecialityRecord }, function(err) {
        if (err) {
...
function addSpecialityService(req, res) {
    ...
    bodydata.serviceDescrip = (req.body.serviceDescrip)? req.body.serviceDescrip:'';
    ...
    ServiceModel.findOne({ serviceName: bodydata.serviceName, isDeleted: false }).exec(function(err, resService) {
        if (err) {
            ...
            SpecialityModel.findOne({ _id: mongoose.Types.ObjectId(bodydata.specialityId) }).exec(function(err, speciality) {
                if (err) {
                    throw err;
                } else {
...
function getSpecialityServiceList(req, res) {
    specialityModel.findOne({ _id: mongoose.Types.ObjectId(req.body.id) }, function(err, specialityData) {
        var serviceArr = [];
...
function specialityServiceDelete(req, res) {
    ServiceModel.remove({ _id: mongoose.Types.ObjectId(req.body.id) }, function(err) {
        if (err) {
            res.json({ code: 201, message: 'Request could not be processed. Please try again.' });
        } else {
            res.json({ code: 200, message: 'Speciality removed  from network.' });
...
function specialityServiceStatus(req, res) {
    ...
    ServiceModel.update({ _id: mongoose.Types.ObjectId(req.body.id) }, { $set: updateSpecialityRecord }, function(err) {
        if (err) {
...
function updatePatientNotificationStatus(req, res) {
    ...
    specialityModel.update({ _id: mongoose.Types.ObjectId(req.body.id) }, { $set: updateSpecialityRecord }, function(err) {
        if (err) {
            ...
            res.json({ code: 200, message: 'Speciality Notification updated successfully.' });
        }
    });
}