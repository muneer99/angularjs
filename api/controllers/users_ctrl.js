...
function validateFrontDeskAccess(req, res) {
  ...
  co(function* () {
    ...
    yield User.findOne(
      {
        _id: mongoose.Types.ObjectId(req.body.reqUser),
        userType: "superAdmin",
      },
      function (err, res) {
        if (!err && res) {
    ...
    User.findOne(
      {
        ...
        userType: { $in: ["user", "network", "officeAdmin"] },
      },
      {
        password: 0,
      ...
      function (err, userInfo) {
        if (!err) {
          ...
          if (
            (userInfo.frontdesk &&
              userInfo.frontdesk.indexOf(req.body.reqUser) > -1) ||
            superAdmin
          ) {
...
function validateTokenAccess(req, res) {
  ...
  co(function* () {
    // Check if it is a super admin access
    yield networkModel.findOne(
      { _id: mongoose.Types.ObjectId(req.body.reqUser), verified: true },
      function (err, res) {
        if (!err && res) {
          network = true;
        }
                  ...
                  function (err) {
                    if (err) {
                      ...
                      co(function* () {
                        ...
                        let usrDegree = userInfo.degree
                          ? yield utility.getTitleById(userInfo.degree)
                          : "";
                        let frmSvpDeg = userInfo.degree
                          ? yield utility.getTitleById(userInfo.degree)
                          : "";
                        let toSvpDeg = userInfo.degree
                          ? yield utility.getTitleById(userInfo.degree)
                          : "";
                        ...
                        var degree =
                          userInfo.degree && userInfo.degree != ""
                            ? ", " + usrDegree
                            : "";
                        var centername = userInfo.centername
                          ? userInfo.centername
                          : "";
                        var replacement = {
                          "{{lastname}}": lastname,
                        ...
                        utility.sendmailbytemplate(
                          ...
                          function (error, templateData) {
                            if (error) {
                              ...
                              userInfoData.doctorStatus = userInfo.doctorStatus;
                              userInfoData.phone_number = userInfo.phone_number;
                              userInfoData.emailAvailable =
                                userInfo.emailAvailable;
                              userInfoData.token = "admin_bearer " + jwtToken;
                              userInfoData.service = userInfo.service
                                ? userInfo.service
                                : "";
                              userInfoData.degree = userInfo.degree
                                ? userInfo.degree
                                : "";
                              userInfoData.network = userInfo.network
                                ? userInfo.network
                                : "";
                              userInfoData.speciality = userInfo.speciality
                                ? userInfo.speciality
          ...
          if (userRecord) {
            ...
            co(function* () {
              ...
              let frmSvpDeg = userRecord.degree
                ? yield utility.getTitleById(userRecord.degree)
                : "";
              let toSvpDeg = userRecord.degree
                ? yield utility.getTitleById(userRecord.degree)
                : "";
...
function addUser(req, res) {
  ...
  var userData = {
    ...
    centername:
      typeof req.body.centername !== "undefined" ? req.body.centername : "",
    email: req.body.hasOwnProperty("email") ? req.body.email.toLowerCase() : "",
    doctorsNpi: req.body.hasOwnProperty("doctorsNpi")
      ? req.body.doctorsNpi
      : "",
    phone_number: req.body.phone_number,
    fax: req.body.hasOwnProperty("fax") ? req.body.fax : "",
    poc_name: req.body.hasOwnProperty("poc_name")
      ? req.body.poc_name.toLowerCase()
      : "",
    ...
    speciality:
      req.body.speciality && req.body.speciality[0] !== ""
        ? req.body.speciality
        : [],
    ...
    createdById: req.body.hasOwnProperty("createdById")
      ? req.body.createdById
      : null,
    service: req.body.hasOwnProperty("service") ? req.body.service : [],
    cell_phone: req.body.hasOwnProperty("cell_phone")
      ? req.body.cell_phone
      : "",
    user_loc: req.body.hasOwnProperty("user_loc")
      ? req.body.user_loc.reverse()
      : [],
  };

  if (req.body.hasOwnProperty("doctorsNPI") && req.body.doctorsNPI)
    userData.doctorsNPI = req.body.doctorsNPI;
  ...
  async.waterfall(
    [
      function (callback) {
        if (utills.notEmpty(userData.doctorsNPI)) {
          User.findOne(
            {
              doctorsNPI: req.body.doctorsNPI,
              deleted: false,
            ...
            function (err, doctorsNPI) {
              if (err) {
                ...
                if (doctorsNPI) {
                  callback({
                    code: 201,
                    message: "Doctors NPI already exist.",
                    data: { userId: doctorsNPI._id },
                  });
                } else {
                  callback(null);
                }
      ...
      function (callback) {
        if (utills.notEmpty(userData.email) || utills.notEmpty(userData.fax)) {
          if (!utills.notEmpty(userData.email)) {
            userData.email =
              Math.floor(10000 + Math.random() * 90000, new Date().getTime()) +
              "_temp@wd.com";
            userData.emailAvailable = 0;
          }
          var UsersRecord = new User(userData);
          console.log("user data ---", UsersRecord);
          //return false;
          ...
          UsersRecord.save(function (err, userRecord) {
            if (err) {
              callback({
                code: 201,
                message: "Doctors NPI id already exists.",
                data: err,
              });
            } else {
              if (userRecord) {
                ...
                if (
                  userData.emailAvailable === 0 &&
                  utills.notEmpty(userRecord.fax) &&
                  userRecord.fax != "+1undefined"
                ) {
                  var dr = "Dr. ";
                  var dataText = "";
                  ...
                  if (
                    !userdata.firstLogin &&
                    req.body.reqfrom &&
                    req.body.reqfrom == "faxAddAndRefer"
                  ) {
                    co(function* () {
                      let refUsrDeg = req.body.referingUserDegree
                        ? yield utility.getTitleById(
                            req.body.referingUserDegree
                          )
                        : "";
                      let userDeg = userRecord.degree
                        ? yield utility.getTitleById(userRecord.degree)
                        : "";
                      ...
                      var faxreplacement = {
                        ...
                        "{{title}}":
                          userRecord.degree && userRecord.degree != "0"
                            ? ", " + userDeg
                            : "",
                        "{{referringProviderFirstname}}": req.body
                          .referingUserFname
                          ? req.body.referingUserFname
                          : "",
                        "{{referringProviderLname}}": req.body.referingUserLname
                          ? req.body.referingUserLname
                          : "",
                        "{{referringProviderTitle}}":
                          ...
                          req.body.referingUserDegree != ""
                            ? ", " + refUsrDeg
                            : "",
                        "{{referringProviderCenter}}": req.body
                          .referingUserCenter
                          ? req.body.referingUserCenter
                          : "",
                        "{{referingUserFnameAgain}}": req.body.referingUserFname
                          ? req.body.referingUserFname
                          : "",
                        "{{webUrl}}": req.config.webUrl,
                        "{{email}}": userInfo.email,
                      ...
                      faxmodel.findOne({ key: templateKey }, {}, function (
                        err,
                        resp
                      ) {
                        if (!err) {
                          if (resp.body) {
                            ...
                            dataText = utility.replaceString(
                              resp.body,
                              faxreplacement
                            );

                            utills.createfaxFile(dataText, function (
                              err,
                              filePath
                            ) {
                              if (!err) {
                    ...
                    req.body.reqfrom == "add_doc_take_outside_referral"
                  ) {
                    co(function* () {
                      ...
                      var faxreplacement = {
                        "{{dr}}": dr,
                        "{{referredtoProviderFirstname}}": req.body.toSvpFname
                          ? req.body.toSvpFname
                          : "",
                        "{{referredtoProviderLastname}}": req.body.toSvpLname
                          ? req.body.toSvpLname
                          : "",
                        "{{referredtoProviderTitle}}":
                          req.body.toSvpDegree && req.body.toSvpDegree != "0"
                            ? ", " + toSvpDeg
                            : "",
                        "{{referringProviderCenter}}": req.body.toSvpCenter
                          ? req.body.toSvpCenter
                          : "",
                        ...
                        "{{referringProviderLname}}": req.body.fromSvpLname
                          ...
                          : "",
                        "{{referringProviderTitle}}":
                          req.body.fromSvpDegree &&
                          req.body.fromSvpDegree != "0"
                            ? ", " + frmSvpDeg
                            : "",
                        "{{referredtoProviderCenter}}": req.body.fromSvpCenter
                  ...
                  } /*if (req.body.hasOwnProperty("createdByAdmin") && req.body.createdByAdmin)*/ else {
                    co(function* () {
                      let userDeg = userRecord.degree
                        ? yield utility.getTitleById(userRecord.degree)
                        : "";
                    ...
                    if (req.body.subject) {
                      ...
                      var replacement = {
                        ...
                        "{{toSvpLname}}": req.body.toSvpLname
                          ...
                          : "",
                        "{{toSvpTitle}}":
                          req.body.toSvpDegree && req.body.toSvpDegree != ""
                            ? ", " + toSvpDeg
                            : "",
                      ...
                      if (userRecord.userType === "officeAdmin") {
                        ...
                        !userdata.firstLogin ||
                        (req.body.hasOwnProperty("createdByAdmin") &&
                          req.body.createdByAdmin)
                      ) {
                        var templateKey = "add_user_static_subject";
                        var salutation = req.body.salutation
                          ? req.body.salutation
                        ...
                        if (
                          ...
                          if (
                            ...
                            co(function* () {
                              ...
                              var logoUrl =
                                "/public/assets/images/logo_new.png";
                              ...
                              var faxreplacement = {
                                ...
                                "{{referringProviderFirstname}}": req.body
                                  ...
                                  ? req.body.referingUserFname
                                  : "",
                                "{{referringProviderLname}}": req.body
                                  .referingUserLname
                                  ? req.body.referingUserLname
                                  : "",
                                "{{referringProviderTitle}}":
                                ...
                                "{{referringProviderCenter}}": req.body
                                  ...
                                  ? req.body.referingUserCenter
                                  : "",
                                "{{referingUserFnameAgain}}": req.body
                                  .referingUserFname
                                  ? req.body.referingUserFname
                                  : "",
                                "{{webUrl}}": req.config.webUrl,
                            ...
                            co(function* () {
                              ...
                              var faxreplacement = {
                                "{{dr}}": dr,
                                "{{referredtoProviderFirstname}}": req.body
                                  .toSvpFname
                                  ? req.body.toSvpFname
                                  : "",
                                "{{referredtoProviderLastname}}": req.body
                                  .toSvpLname
                                  ? req.body.toSvpLname
                                  : "",
                                "{{referredtoProviderTitle}}":
                                  req.body.toSvpDegree &&
                                  req.body.toSvpDegree != "0"
                                    ? ", " + toSvpDeg
                                    : "",
                                "{{referringProviderCenter}}": req.body
                                  .toSvpCenter
                                  ? req.body.toSvpCenter
                                  : "",
                                "{{referringProviderFname}}": req.body
                                  .fromSvpFname
                                  ? req.body.fromSvpFname
                                  : "",
                                "{{referringProviderLname}}": req.body
                                ...
                                "{{referringProviderTitle}}":
                                  ...
                                  req.body.fromSvpDegree != "0"
                                    ? ", " + frmSvpDeg
                                    : "",
                                "{{referredtoProviderCenter}}": req.body
                                  .fromSvpCenter
                                  ? req.body.fromSvpCenter
                                  : "",
                                "{{mailBody}}": req.body.mailBody,
...
function updateUserEmail(req, res) {
  User.findOne(
    { email: req.body.email, deleted: false, isOutside: false },
    { email: 1 },
    function (err, email) {
      if (err) {
        res.json({
...
function changePass(req, res) {
  ...
  User.findOne({ passwordResetToken: token }, { _id: 1 }).exec(function (
    err,
    data
  ) {
    if (err) {
      ...
      //return false
      new User(data).save(function (err, userData) {
        if (err) {
          res.json(Error(constant.statusCode.error, err));
        } else {
          res.json({
  ...
  User.findOneAndUpdate(
    ...
    function (err, resp) {
      if (err) {
        ...
        res.json({ code: 201, message: "Invalid token.1111" });
      } else {
        if (resp) {
          res.json({
            code: 200,
          ...
          res.json({ code: 201, message: "Invalid token.1212" });
        }
      }
    }
  );
...
function changePassAdmin(req, res) {
  ...
  User.findOne(
    ...
    if (err) {
      ...
      res.json({
        code: 201,
        message: constant.validateMsg.internalError + " " + err,
      });
    } else if (data._id) {
      data.password = Password;
...
function resetPassword(req, res) {
  User.findOne(
    { _id: mongoose.Types.ObjectId(req.body.userId) },
    { email: 1, lastname: 1, firstname: 1, degree: 1 },
    function (err, users) {
      if (err) {
        res.json({
          code: 201,
          ...
          User.update(
            ...
            function (err) {
              if (err) {
                ...
                co(function* () {
                  let usrDeg = users.degree
                    ? yield utility.getTitleById(users.degree)
                    : "";
                  var replacement = {
                    "{{firstname}}": users.firstname ? users.firstname : "",
                    "{{lastname}}": users.lastname ? users.lastname : "",
                    "{{title}}":
                      users.degree && users.degree != "" ? ", " + usrDeg : "",
                    "{{webUrl}}": req.config.webUrl,
                    "{{gpassword}}": gpassword,
                    "{{currentYear}}": currentYear,
                  };
...
function getUserList(req, res) {
  User.find(
    { deleted: false, userType: { $in: ["user"] } },
    {
      _id: 1,
      firstname: 1,
      lastname: 1,
...
function getFrontDeskAdmin(req, res) {
  User.find(
    { deleted: false, isVerified: true, userType: { $in: ["officeAdmin"] } },
    {
      _id: 1,
      firstname: 1,
      lastname: 1,
...
function UpdateContactDetails(req, res) {
  ...
  if (!req.body.degree) delete req.body.degree;

  if (!req.body.officeadminTitle) delete req.body.officeadminTitle;
      ...
      if (err) {
        ...
        async.each(
          ...
          function (item, callback) {
            User.findOne({ _id: mongoose.Types.ObjectId(item) }, function (
              err,
              frontDeskData
            ) {
              if (err) {
  ...
  User.count(
    ...
    function (err, count) {
      if (err) {
        ...
        referralsModel.count({}, function (err, count) {
          if (err) {
            res.json({
              code: 201,
              message: "Error",
                ...
                if (err) {
                  ...
                  networkModel.count({}, function (err, resCount) {
                    if (err) {
                      ...
                      serviceModel.count({ isDeleted: false }, function (
                        err,
                        serviceCount
                      ) {
                        if (err) {
                  ...
                  async.eachSeries(
                    ...
                    function (item, callback) {
                      co(function* () {
                        ...
                        var prefRatData = new userPreferenceRatingModel(
                          prefRat
                        );
  ...
  try {
    ...
    if (req.body.hasOwnProperty("emailtype")) {
      ...
      // saurabh 06-June-2019
        ...
        condition.$or = [{ emailAvailable: { $ne: 0 } }, { emailAvailable: 0 }];
      }
    }
          ...
          if (err) {
            ...
            if (req.body.network) {
              for (let i = 0; i < userData.length; i++) {
                ...
                yield userNetwork.find(findUserNetworkData, function (
                  err,
                  userNetworkData
                ) {
                  if (err) {
            ...
            User.count(condition).exec(function (err, userDataCount) {
              if (err) {
                ...
                finalUserData.forEach(function (item, index) {
                  ite++;
                  referModel.count(
                    { referredTo: item._id, status: 0 },
                    function (err, res) {
                      finalUserData[index].inboxCount = res;
                    }
                  );
                  referModel.count(
                    { referredBy: item._id, status: 3 },
                    function (err, res) {
  ...
  try {
    ...
    if (req.body.hasOwnProperty("emailtype")) {
      if (req.body.emailtype == "with_mail") {
        condition.$and = [
          {
            $or: [{ email: { $ne: "" } }, { email: { $not: /temp@wd.com$/ } }],
          },
        ...
        condition.$and = [
          {
            $or: [{ email: { $eq: "" } }, { email: { $eq: /temp@wd.com$/ } }],
          },
              ...
              if (err) {
                ...
                setTimeout(function () {
                  return res.json({
                    ...
                    data: finalUserData,
                    totalCount:
                      finalUserData.length == userData.length
                        ? userDataCount
                        : finalUserData.length, //((userDataCount[0]) ? userDataCount[0].count : 0)
                  });
                }, 1000);
              }
            });
...
function getDoctorRatingList(req, res) {
  ...
  let aggregate = [];
  if (
    req.body.hasOwnProperty("user_loc") &&
    req.body.user_loc &&
    req.body.user_loc.length > 0
  ) {
  ...
  User.aggregate(aggregate).exec(function (err, userData) {
    if (err) {
      ...
      User.aggregate(aggregateCnt).exec(function (err, userDataCount) {
        if (err) {
          res.json({
            code: 201,
            message: "internal error.",
          ...
          userData.forEach(function (item, index) {
            ...
            let ratingusercnt =
              userData[index].preference_rating[0].userIds.length;

            let totalPrefRating = userData[
              index
            ].preference_rating[0].totalRating.reduce((a, b) => a + b, 0);
        ...
        function (err, userRecord) {
          if (err) {
            ...
            if (userRecord) {
              ...
              if (
                req.body.flag == "superadmin" ||
                req.body.flag == "insuranceadmin"
              ) {
                req.body.status = "0";
              }
              var userNetworkArr = new userNetwork({
...
function updateProviderNetwork(req, res) {
  ...
  async.waterfall(
    [
      function (callback) {
        User.find(condition1).exec(function (err, data) {
          if (err) {
            callback({
              code: 201,
      ...
      function (callback) {
        User.find(condition2).exec(function (err, data) {
          if (err) {
            callback({
              code: 201,
            ...
            data.forEach((item) => {
              ...
              item.network.splice(
                item.network.indexOf(
                  mongoose.Types.ObjectId(req.body.networkId)
                ),
                1
              );
            ...
            if (req.body.network.length) {
              for (let i = 0; i < userRecord.length; i++) {
                ...
                for (let j = 0; j < req.body.network.length; j++) {
                  console.log(
                    userRecord[i].network,
                    req.body.network[j]._id,
                    userRecord[i].network == req.body.network[j]._id,
                    "userRecord[i].network == req.body.network[j]",
                    i,
                    j
                  );
                  ...
                  if (j == req.body.network.length - 1) {
                    userNetwork.updateMany(
                      {
                        _id: mongoose.Types.ObjectId(userRecord[i]._id),
                        network: mongoose.Types.ObjectId(userRecord[i].network),
                      },
                      status,
                      function (err, data) {
                        console.log(
                ...
                if (i == userRecord.length - 1) {
                  ...
                  req.body.network.forEach((element, index) => {
                    ...
                    if (index == req.body.network.length - 1) {
                      ...
                      userNetwork.updateMany(findObj, { status: 0 }, function (
                        err,
                        data
                      ) {
                        if (err) {
...
function migrateData(req, res) {
  User.find(
    { network: { $exists: true }, $where: "this.network.length > 0" },
    function (err, result) {
