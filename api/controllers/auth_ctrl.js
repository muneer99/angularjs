...
function userRegister(req, res) {
  if (req.body.email && !validator.isEmail(req.body.email)) {
    ...
    User.findOne(
      { userid: req.body.userid, doctorsNPI: req.body.npinumber },
      { _id: 1 }
      ...
      if (err) {
        ...
        if (data && data._id) {
          return res.json({
            code: 401,
            message: "User id and NPI number already exist. Try with another !",
            data: [],
          });
        } else {
          User.existCheck(req.body.email, "", function (err, exist) {
            if (err) {
              ...
              if (exist != true) {
                // console.log("=== = body jjjjj",req.body,exist);
                // return false;
                ...
                new User(obj).save(function (err, userData) {
                  if (err) {
                    ...
                    // console.log("==== body gg email ",req.body,obj);
...
function userLogin(req, res) {
  ...
  if (!req.body.email || !req.body.password) {
    ...
    return res.json(Response(402, "Invalid user id"));
  } else {
  }
    ...
    if (err) {
      ...
      if (userInfo != null) {
        if (userInfo.status == "0") {
          ...
          userInfo.comparePassword(req.body.password, async function (
            err,
            isMatch
          ) {
            ...
            if (err) {
              ...
              userTokenArr.save(function (err) {
                if (err) {
                  ...
                  if (
                    dateOfOrg &&
                    dateOfOrg.getTime() > currentDate.getTime()
                  ) {
...
function updatePassword(req, res) {
  ...
  var passEnc = utility.getEncryptText(req.body.password);
  var newPassword = utility.getEncryptText(req.body.confirm_password);
  var userData = {
    //email: req.body.email.toLowerCase(),
    password: passEnc,
  };
...
function adminLogin(req, res) {
  ...
  if (validator.isNull(req.body.email) || !req.body.email) {
    ...
    User.findOne({ email: req.body.email, userType: "superAdmin" }).exec(
      function (err, admin) {
        ...
        if (err) {
          ...
          if (!admin) {
            ...
            admin.comparePassword(passEnc, async function (err, isMatch) {
              if (err) {
                res.json({
                  code: 402,
                  message: "Something went wrong" + err,
