          ...
          if (err) {
            ...
            if (req.body.network) {
              for (let i = 0; i < userData.length; i++) {
                ...
                yield userNetworkModel.find(findUserNetworkData, function (
                  err,
                  userNetworkData
                ) {
                  if (err) {
                            ...
                            UsersRecord.save(function (err, data) {
                              if (err) {
                                ...
                                if (data) {
                                  ...
                                  var userNetworkRecord = new userNetworkModel(
                                    objToAdd
                                  );
                                  userNetworkRecord.save(function (
                                    error,
                                    userNetwrkData
                                  ) {
                                    if (error) {
                                      console.log(
                                        "\n\n userNetwrkData error",
                                        error
                                      );
                                    } else {
...
function addDoctorInNetwork(req, res) {
  console.log("muneer doc add=====", req.body);
...
function getdoctors(req, res) {
  ...
  var searchText = req.body.search
    ? req.body.search.firstName
      ? req.body.search.firstName.toLowerCase()
      : ""
    : "";
  if (searchText !== "") {
    cond = {
    ...
    function (err, prefrenceInfo) {
      if (err) {
        ...
        if (req.body.userLoc && req.body.range) {
          ...
          UserModel.find(condition, { password: 0 }).exec(function (
            err,
            userInfo
          ) {
            if (err) {
