...
nwdApp.controller("frontdesktitleController", [
    '$scope',
    '$rootScope',
    '$sessionStorage',
    '$location',
    ...
    function(
        ...
        $scope.searchable = function(searchTextField) {
            ...
            $scope.tableParams = new ngTableParams(ngTableParamsService.get(), {
                getData: function($defer, params) {
                    ...
                    $scope.dataList     = [];
                    frontdesktitleService.getTitleList().save($scope.paramUrl, function(response) {
        ...
        $scope.getTitleList = function() {
            ...
            $scope.tableParams = new ngTableParams(ngTableParamsService.get(), {
                getData: function($defer, params) {
                    ...
                    $scope.tableLoader  = true;
                    $scope.dataList     = [];
                    frontdesktitleService.getTitleList().save($scope.paramUrl, function(response, err) {
        ...
        */
        $scope.addTitle = function(data) {
            var actvLog = {userId:localData._id, type: 5, detail: 'Add title / degree' + data.name};
            if(data.hasOwnProperty('_id') && data._id!=''){
                actvLog.detail  = 'Update title / degree' + data.name;
                actvLog.type    = 6;
            }
            frontdesktitleService.addTitle().save(data, function(response) {
                if (response.code == 200) {
                    actvLog.success = true;
                    logProvider.addUserActivity().save(actvLog, function(res){});
                    logger.logSuccess(response.message);
                    $state.go('frontdesktitle', {}, {reload: true});
                } else {
                    logProvider.addUserActivity().save(actvLog, function(res){});
                    logger.logError(response.message);
                }
            ...
            swal({
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        var serviceData = {};
                        serviceData.id  = id;
                        frontdesktitleService.deleteTitle().save(serviceData, function(response) {
                            if (response.code == 200) {
        ...
        */
        $scope.getTitle = function() {
            ...
            if ($state.params.id) {
                ...
                frontdesktitleService.getTitle().get({ id: $state.params.id }, function(response) {
                    if (response.code == 200) {
                        actvLog.success = true;
                        logProvider.addUserActivity().save(actvLog, function(res){});
