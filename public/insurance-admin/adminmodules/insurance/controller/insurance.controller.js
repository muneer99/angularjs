    ...
    function (
           ...
           // console.log(" data ", data);
            insuranceService.addNetwork().save(data, function (response) {
                if (response.code == 200) {
                    ...
                    var test = $window.sessionStorage.test;
                    //console.log(" test ", test);

                    localData.name = data.name;
                    localData.email = data.email;
                    localData.desc = data.desc;

                    $window.sessionStorage.setItem('test', JSON.stringify(localData));

                    var test2 = $window.sessionStorage.test;

                   // console.log(" test2 ", test2);
                    $state.go('insurancedashboard', {}, { reload: true });
                    ...
                    doctorService.getDoctorsListAssociatedInsurance().save($scope.paramUrl, function (response, err) {
                        ...
                        if (response.code == 200) {
                            ...
                            /* response.data.forEach(function (item, index, object) {
                                item.user_network.forEach(function (item2, index2, object2) {
                                    item2.userNetworkInfo.forEach(function (item3, index3, object3) {
                                        //console.log(" userNetworkInfo ", item3.network);
                                        if (item3.network != $state.params.id) {
                                            object3.splice(index3, 1);
                                        }
                                    });
                                });
                            }); */
            ...
            if ($scope.doctorList.length > 0) {
                ...
                if ($scope.IdArray.length > 0) {
                    ...
                    if (user) {
                        if ((user.fromName) && (user.fromEmail)) {
                            if (user.fromName.length > 0 && user.fromEmail.length > 0) {
                                insuranceService.sendEmailNetworkInsurance().save({ provider: $scope.IdArray, insuranceProvider: $state.params.id, senderName: user.fromName, senderEmail: user.fromEmail }, function (resp) {
                                    $rootScope.loading = false;
                                    // console.log(" resp ",resp);                                    
                                    logger.logSuccess('Email Sending process inprogress');
                                })
        ...
        */
        ...
        $scope.updateProviders = function (data) {
            ...
            doctorService.updateProviderNetwork().save({ networkId: data.insId, netProvs: data.insprovider }, function (response) {
                ...
                if (response.code == 200) {
                    ...
                    // $state.go('insurance', {}, { reload: true });
                    $state.go('viewInsProviderAll', { id: data.insId }, { reload: true });
                } else {
        ...
        $scope.updateProvidersUnlisted = function (data) {
            // console.log(" updateProvidersUnlisted data ", data);
            if (data.selectesList.length > 0) {
                //doctorService.updateProviderNetwork().save({ networkId: data.insId, netProvs: data.insprovider }, function (response) {
                doctorService.updateProviderNetworkUnlisted().save({ networkId: data.insId, netProvs: data.selectesList,flag:'insuranceadmin' }, function (response) {
                    // console.log(" response ", response);
                    if (response.code == 200) {
                        swal("Updated!", "Providers within the network updated.", "success");
                        $state.go('viewInsProviderAll', { id: data.insId }, { reload: true });
        ...
        $scope.$watch('nonRegData', function () {
            console.log(" inside nonRegData", nonRegData);
